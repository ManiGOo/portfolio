...
export default function SectionHeading({ index, eyebrow, title, lede, align = "left" }) {
  ...
  return (
    <div className={`${centered ? "text-center" : "text-left"} mb-14 md:mb-20`}>
      ...
      >
        <span className="mono-label text-emerald-400/90">{index}</span>
        <span className="h-px w-10 bg-emerald-400/40" aria-hidden="true" />
      ...
      <WordReveal
        as="h2"
        text={title}
        className="font-display display-tight text-balance font-bold text-4xl sm:text-5xl md:text-6xl text-zinc-50"
      />
      ...
      {lede && (
        <motion.p
          ...
          className={`mt-6 max-w-2xl text-base md:text-lg leading-relaxed text-zinc-400 ${centered ? "mx-auto" : ""}`}
        >
          {lede}
        </motion.p>
      )}
